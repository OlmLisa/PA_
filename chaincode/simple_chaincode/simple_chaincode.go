...
func (cc *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	function, args := stub.GetFunctionAndParameters() // une fonction peut renvoyer plusieurs éléments
	logger.Debugf("function: %s", function)

	if function == "put" {
		return cc.put(stub, args) // cc = this. (chaincode en question)
...
func (cc *SimpleChaincode) put(stub shim.ChaincodeStubInterface, args []string) pb.Response { // la clé du state et la valeur qu'on veut mettre pour cette clé
...
func (cc *SimpleChaincode) get(stub shim.ChaincodeStubInterface, args []string) pb.Response { // structure = stub; qui prends que une clé qui est le premier arg
